...
class Datahandler{
	...
	public function loadCourseName(){
		$output ="";
		$tagBus =0;$tagRetail=0; $tagTourism=0; $tagSkill=0;
		$bus = "Business";
		$ret = "Retail";
		$tour = "Tourism";
		$skill = "Skills";
		foreach ($this->courseData as $key => $value) {
			if(strpos($value->courseName,$bus) && !$tagBus){
				$output.='<option disabled>'.$bus.'</option>';
				$tagBus=1;
			}
			else if(strpos($value->courseName,$ret) && !$tagRetail){
				$output.='<option disabled>&nbsp</option>';
				$output.='<option disabled>'.$ret.'</option>';
				$tagRetail=1;
			}
			else if(strpos($value->courseName,$tour) && !$tagTourism){
				$output.='<option disabled>&nbsp</option>';
				$output.='<option disabled>'.$tour.'</option>';
				$tagTourism=1;
			}
			else if(strpos($value->courseName,$skill) && !$tagSkill){
				$output.='<option disabled>&nbsp</option>';
				$output.='<option disabled>'.$skill.'</option>';
				$tagSkill=1;
			}
